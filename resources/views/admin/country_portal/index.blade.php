                    ...
                    <div class="card-body d-flex justify-content-end align-items-end">
                        ...
                        <form action="{{ route('admin.country-portals.index') }}" method="GET" class="d-flex me-3">
                            <input type="text" name="search" class="form-control me-2"
                                placeholder="Search by country name, code, or language" value="{{ $search }}">
                                    ...
                                    @foreach ($countryPortals as $index => $country)
                                        <tr>
                                            <th scope="row" class="text-center">
                                                {{ $countryPortals->firstItem() + $index }}</th>
                                            <td>
                                                <strong>{{ $country->country_name }}</strong>
                                                @if ($country->analytics_slug)
                                                    <br><small class="text-muted">Slug:
                                                        {{ $country->analytics_slug }}</small>
                                                @endif
                                            </td>
                                            <td>
                                            ...
                                            <td class="text-center">
                                                <div class="btn-group" role="group">
                                                    <a href="{{ route('admin.country-portals.show', $country->id) }}"
                                                        class="btn btn-sm btn-info" title="View">
                                                        <i class="bx bx-show"></i>
                                                    </a>
                                                    <a href="{{ route('admin.country-portals.edit', $country->id) }}"
                                                        class="btn btn-sm btn-warning" title="Edit">
                                                        <i class="bx bx-edit"></i>
                                                    </a>
                                                    <form
                                                        action="{{ route('admin.country-portals.destroy', $country->id) }}"
                                                        method="POST" class="d-inline delete-form">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" class="btn btn-sm btn-danger" title="Delete"
                                                            onclick="return confirm('Are you sure you want to delete this country portal?')">
